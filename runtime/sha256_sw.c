...
/* Wrapper for XIOM FFI: matches xiom_sha256_sw_compress interface.
   Takes raw uint32_t state[8] and uint8_t block[64] pointers. */
void xiom_sha256_sw_compress(unsigned int state[8], const unsigned char block[64]) {
    SHA256_CTX ctx;
    int i;
    for (i = 0; i < 8; i++) ctx.state[i] = state[i];
    sha256_transform(&ctx, block);
    for (i = 0; i < 8; i++) state[i] = ctx.state[i];
}




/* One-shot SHA-256: computes hash of input data, writes 32-byte digest to output.
   Both input and output are raw byte buffers. */
void xiom_sha256_hash(const unsigned char* input, unsigned int input_len, unsigned char* output) {
    SHA256_CTX ctx;
    sha256_init(&ctx);
