...
> (bug status), `docs/stdlib_session.md` §5b (dependency order L0–L4).
...
| BUG 14 (UInt64→UInt128 sext / UInt128 ashr) | **FIXED** | clean 64×64→128 mulhi available (no 32-bit-half hack needed) |
...
| **BUG 2** (module-global struct FIELD writes lost) | **OPEN** (advisory) | **AVOID** `g.field = x` on module globals — use whole-value assignment `g = Struct{...}`. Only matters for precision-cached π/ln10 and `const BIGINT_*` |
...
| §7 NASM/SIMD tracks (math/crypto/hash/compress asm, CPUID dispatch) | OPEN — **off-limits to the stdlib session** (`crates/` + `stdlib/runtime/*.c` are the compiler session's files) | implement **pure-XIOM optimized** first; add `unsafe { asm }`/SIMD hot paths only if/once the compiler session exposes the §7 track, or coordinate with them. The `unsafe { extern }` C-FFI path (ffi.xi, libm) IS available for hot numeric loops with zero external deps (libm is the platform C runtime). |
...
5. **Performance.** Prefer pure-XIOM optimized algorithms (match the "expected"
   ...
   §7 (see above) — write the pure fallback + a clean seam for the dispatch.
...
7. **No generic abuse.** Follow `docs/STDLIB_GENERICS.md` R1–R9. Re-check each
   "blocked" stub now that BUG 12/15/17 are fixed — most are unblocked. The §12
...
- Each agent is given: the sublib's stub file, `docs/STDLIB_IMPLEMENTATION.md`
  ...
  §3 home note, and the module's existing real code (to match style + reuse).
- Each agent must:
  1. Implement every stub `// fn` in its file(s) to the §2 standard.
...
5. Re-run gates after each multi-module batch (see §4).
...
1. **BUG 2** — module-global struct FIELD writes lost (unblocks precision-cached
   π/ln10 and `const BIGINT_*/BIGFLOAT_*`).
2. **BUG 3** — module-global fn-call initializers zero (same unlocks).
3. **§7 NASM/SIMD** — expose the asm/CPUID track so hot math/hash/crypto can use
   it (stdlib session writes the pure fallbacks + dispatch seam).
4. **§12 interface dispatch** — trait-based generic math (`impl Num[T]`)
