...
| Commit | Content |
|--------|---------|
| `c9426687` | **MISSION 1 -- wish-list audit** (docs/STDLIB_EXTENSION.md S13): full categorized HAVE/GAP-stdlib/GAP-package tables for HASHING/COLLECTIONS/STRING/CONVERSION/NETWORK/FILE FORMATS/OS + dedupe map; S10 status refreshed (64+ modules, ~2,215 fns, Phases A-C.5/Karatsuba). |
...
| docs (uncommitted -> final commit) | AI_CONTEXT S8.3/8.5/8.19/8.45/8.46/8.44 additions, STDLIB_EXTENSION S13.9 generics policy, this session doc. |
...
S13 now contains the full categorized audit (HAVE / GAP-stdlib / GAP-package
tables for all 7 wish-list categories + dedupe map + S13.9 generics policy).
Remaining GAP-stdlib work by priority (from S13):
...
- **STDLIB**: implementable with **ZERO external dependencies** -- pure XIOM,
  plus SIMD/C-FFI/ASM **for performance/optimization only** (the S7 pattern:
...
- **PACKAGES** (ignore for the stdlib audit): anything that **requires an
  ...
  bindings, openssl, etc. (the FFI-bound 51 in S8.1 + the placeholders that
  wrap C libs).
...
- Existing folder categories (D4 pattern): `collect/`, `format/`, `hash/`,
  `math/`, `net/`, `num/`, `os/`, `rand/`, `text/` (see S5 for contents).
...
1. **Compiler bugs to land first** (they block/constrain stdlib shape):
   ...
   mask drop), BUG 16 (skiplistxtrie / multi-Option-payload startup
   fast-fail), BUG 17 (Vec[Str] element == -> pointer cmp), BUG 18
   (stringxtext.similarityxtime combo crash; %Q strptime pair miscompile).
...
7. **NASM/SIMD S7 tracks**: compiler/runtime session domain (crates/ +
   stdlib/runtime/*.c are OFF-LIMITS to us). BUG 2/3 fix would enable
   precision-cached pi/ln10 in bigfloat.
...
- **UInt64->UInt128 casts sext and UInt128 `>>` is ashr** (BUG 14): the XXH3
  64x64->128 product builds from 32-bit halves and masks the high-half shift.
...
- **Vec-of-struct x 2 in one program -> startup fast-fail** (BUG 16): use
...
**Verified GAPs (from S6 wish-list):** printf/scanf (G13), murmur3_128,
...
(see docs/STDLIB_AUDIT.md S6 checklist).
...
advisory, avoid via whole-value assignment + constructor fns) and the S7
...
1. Order: L0->L4 (see S5b) -- string + math.tower first; follow each stub's `// Depends on:`.
...
4. Use qualified calls (`math.tower.sqrt`) in user code; same-name pairs (37) are
   disambiguated by module path (see STDLIB_AUDIT.md S3 for designated homes).
...
>
> READ FIRST, IN ORDER:
> 1. docs/stdlib_session.md -- full state + S5b dependency order + S6b phase-2 rules
...
> 3. docs/STDLIB_AUDIT.md -- the doc->tree map + same-name home table + S6 checklist
> 4. docs/STDLIB_GENERICS.md -- R1-R9 generic/concrete policy
> 5. docs/COMPILER_BUGS.md STATUS SUMMARY -- BUG 12-18 are FIXED; BUG 2/3 + S7 asm open
>
...
>   math/constants + num/ (see STDLIB_IMPLEMENTATION.md S3 order).
...
>   S7 seam -- pure fallback + dispatch). Follow STDLIB_GENERICS R1-R9.
...
- **STILL OPEN (compiler session):** BUG 2 (module-global struct FIELD writes
  ...
  sext + UInt128 ashr, single-var inline mask drop, skiplistxtrie startup
  fast-fail (unqualified MaybeUninit.clone), Vec[Str] element == -> pointer
  cmp, stringxtext.similarityxtime combo crash (+ %Q strptime pair).
...
- The session doc constraint: only `stdlib/xiom/**`, `examples/**`,
  `docs/COMPILER_BUGS.md`, `docs/AI_CONTEXT.md` S8 are ours to commit.
