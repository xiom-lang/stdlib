// XIOM - Collections: Open-Addressing IntMap
...
// ============================================================================
// Hash IntMap with open addressing over Int keys and Int values.
//
...
/// An open-addressing hash IntMap from Int keys to Int values.
pub type IntMap = {
  keys: Vec[Int];
  values: Vec[Int];
  states: Vec[Int];
  ...
  ensures: result.used == 0
{
  ...
  IntMap{ keys: keys; values: values; states: states; used: 0; }
}

/// Bucket index for a key.
fn bucket_for(m: &IntMap, key: Int) -> Int {
  var h = key;
  if h < 0 {
    h = -h;
  };
...
fn put_internal(m: &mut IntMap, key: Int, value: Int) {
  let cap = m.states.len();
  var idx = key;
  if idx < 0 {
    idx = -idx;
...
pub fn map_get(m: &IntMap, key: Int) -> Option[Int] {
  let cap = m.states.len();
  var idx = key;
  if idx < 0 {
    idx = -idx;
...
/// Complexity: O(1) average.
pub fn map_contains(m: &IntMap, key: Int) -> Bool {
