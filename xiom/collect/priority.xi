...
fn ph_swap(q: &mut IntMaxHeap, a: Int, b: Int) {
  var tmp = q.data[a];
  q.data[a] = q.data[b];
  q.data[b] = tmp;
}

fn ph_sift_up(q: &mut IntMaxHeap, i: Int) {
  var idx = i;
  while idx > 0 {
    var parent = (idx - 1) / 2;
...
fn ph_sift_down(q: &mut IntMaxHeap, i: Int) {
  var idx = i;
  var len = q.data.len();
  loop {
    var l = idx * 2 + 1;
...
pub fn pqueue_new() -> IntMaxHeap {
  return IntMaxHeap{ data: Vec[Int].new(); };
}

/// Insert `value` into the queue. O(log n).
pub fn pqueue_push(q: &mut IntMaxHeap, value: Int) {
  q.data.push(value);
...
/// O(log n).
pub fn pqueue_pop(q: &mut IntMaxHeap) -> Option[Int] {
...
/// O(1).
pub fn pqueue_peek(q: &IntMaxHeap) -> Option[Int] {
  if q.data.len() == 0 { return None; }
  return Some(q.data[0]);
}

/// Number of elements in the queue. O(1).
pub fn pqueue_len(q: &IntMaxHeap) -> Int
  ensures: result >= 0
{
  var len = q.data.len();
  return len;
...
pub fn pqueue_is_empty(q: &IntMaxHeap) -> Bool
